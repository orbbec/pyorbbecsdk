...
Nine numbered tutorials, designed to be **run in order: 01 → 02 → … → 08**, plus one standalone firmware tool.
...
| 03 | `beginner/03_color_and_depth_aligned.py` | Color + depth streams simultaneously; `AlignFilter` for depth-to-color projection; side-by-side display | numpy, opencv |
...
| — | `beginner/device_firmware_update.py` | OTA firmware upgrade from a `.bin` file; progress callback; safe upgrade practices | none |
...
|--------|-------------|-------|
| `advanced/recorder.py` | Record all streams to `.bag` with GUI; pause/resume; multi-panel live view | All |
| `advanced/record_no_gui.py` | Headless recording — no display, prints FPS per stream | All |
| `advanced/playback.py` | Replay a `.bag` file; auto-loop; dynamic multi-stream grid | All |
| `advanced/save_image_to_disk.py` | Capture a fixed number of frames and save as PNG / 16-bit PNG | All |

### Device & System

| Script | Description | Notes |
|--------|-------------|-------|
| `advanced/enumerate.py` | List all devices, sensors, and every supported stream profile interactively | All |
| `advanced/hot_plug.py` | Register a callback to detect camera connect / disconnect events | All |
| `advanced/control.py` | Enumerate all device properties (bool/int/float), check permissions, get/set values | All |
| `advanced/metadata.py` | Read per-frame metadata: exposure time, gain, timestamp, sensor temperature | All |

### Depth Processing & Alignment

| Script | Description | Notes |
|--------|-------------|-------|
| `advanced/custom_filter_chain.py` | Chain Temporal + Spatial + HoleFilling + Threshold filters; tune live with keyboard | All |
| `advanced/hw_d2c_align.py` | Hardware D2C alignment — on-device projection (lower CPU); toggle SW/HW at runtime | All |
| `advanced/post_processing.py` | Full post-processing stack; raw vs. filtered side-by-side comparison | Gemini 330 |
| `advanced/hdr.py` | HDR merge: combine alternating-exposure frames with `HdrMergeFilter` | Gemini 330 |
| `advanced/preset.py` | Load named depth presets (`Default`, `Hand`, `High Accuracy`); switch at runtime | Gemini 330 |
| `advanced/depth_work_mode.py` | Switch depth work modes (High Accuracy, High Density, …) at runtime | Gemini 2 series |
| `advanced/confidence.py` | Access depth confidence map; visualize and threshold per-pixel confidence | Select devices |

### Multi-Device & Network

| Script | Description | Notes |
|--------|-------------|-------|
| `advanced/two_devices_sync.py` | Two cameras simultaneously; hardware frame sync via JSON config | All |
| `advanced/high_performance_pipeline.py` | Async callback pipeline; bounded frame queue; FPS + latency measurement | All |
| `advanced/coordinate_transform.py` | 2D↔2D, 2D↔3D, 3D↔3D, 3D↔2D transforms using calibration API; press 1–4 | All |
| `advanced/laser_interleave.py` | Laser interleave mode to reduce multi-camera IR interference | Select devices |
| `advanced/forceip.py` | Assign a static IP to a network camera | Femto Mega, Gemini 2 XL |
| `advanced/device_optional_depth_presets_update.py` | Write optional depth preset profiles to device | Gemini 330 |
...
Multi-device / sync?  →  advanced/two_devices_sync.py
High performance?     →  advanced/high_performance_pipeline.py
