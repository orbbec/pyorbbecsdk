...
class GlobalState:
    def __init__(self):
        ...
        self.cached_frames = {
            ...
            'left_ir': None, 'right_ir': None, 'ir': None,
            'confidence': None,
...
def setup_camera(file_path: str):
    ...
    for i in range(len(sensor_list)):
        ...
        if sensor_type in video_sensors:
            if (is_astra_mini_device(device_info.get_vid(), device_info.get_pid())
...
def _create_imu_panel(imu_frame, title, w=480, h=240):
    ...
    for i, line in enumerate(lines):
        ...
        cv2.putText(panel, line, ((w - sz[0]) // 2, start_y + i * lh + sz[1]),
...
def render_frames():
    ...
    KEYS = ['color', 'depth', 'left_ir', 'right_ir', 'ir',
            'confidence', 'accel', 'gyro', 'left_color', 'right_color']

    while not state.stop_rendering:
        with state.frame_mutex, state.imu_mutex:
            blocks = [state.cached_frames[k] for k in KEYS
