...
from pyorbbecsdk import Config, OBFrameType
...
def main():
    ...
    pipeline = Pipeline()
    try:
        profile_list = pipeline.get_stream_profile_list(OBSensorType.LEFT_IR_SENSOR)
        try:
    ...
    while True:
        try:
            ...
            if frames is None:
                continue
            ir_frame = frames.get_frame_by_type(OBFrameType.LEFT_IR_FRAME)
            if ir_frame is None:
                continue
