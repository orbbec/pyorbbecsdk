...
def main():
    ...
    bag_player = pipeline.get_bag_player()
    bag_player.set_playback_state_callback(playback_state_callback)
    device_info = pipeline.get_device_info()
    print("Device info: ", device_info)
    camera_param = pipeline.get_camera_param()
    print("Camera param: ", camera_param)
    bag_player.start(frame_playback_callback)
    time.sleep(5)
    while True:
        if bag_player.get_playback_state() == OBMediaState.OB_MEDIA_END:
            break
        time.sleep(1)
    bag_player.stop()
