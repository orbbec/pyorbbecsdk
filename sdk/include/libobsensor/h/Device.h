...
const char *ob_device_list_get_device_ip(ob_device_list *list, uint32_t index, ob_error **error);

int ob_device_list_get_device_port(ob_device_list *list, uint32_t index, ob_error **error);

const char *ob_device_list_get_device_net_name(ob_device_list *list, uint32_t index, ob_error **error);
 ...
 *
 * @param[in] list Device list object
 * @param serial_number Creat the serial number of the device
 * @param[out] error Log error messages
 * @return ob_device* returns the created device
 */
 ...
 *
 * @param[in] list Device list object
 * @param uid Creat the uid of the device
 * @param[out] error Log error messages
 * @return ob_device* returns the created device
 */
 ...
 * @param[in] device Device object
 * @param[in] mode_name Depth work mode name which equals to ob_depth_work_mode.name
 * @param[out] error Log error messages
 *
 * @return Return switch result. OB_STATUS_OK: success, other failed.
 */
 ...
 * @param[in] pBuffer 要发送的数据buffer
 * @param[in] nSize 要发送的数据buffer大小
 ...
 * @brief 接收数据包
 * @param[in] device 设备对象
 * @param[in] pBuffer 要接收的数据buffer
 * @param[in] nReceived 要接收的数据buffer大小
 * @param[out] error 记录错误信息
 * @return 接收状态
 * \endif
 */
