...
public:
    explicit StreamProfile(std::unique_ptr<StreamProfileImpl> impl);
...
public:
     ...
     * the first one in the list will be returned by default. If no matched profile found, will throw exception!
     *
     * @param fullScaleRange fullScaleRange. If no matching condition is required, it can be passed to 0
     * @param sampleRate sampleRate. If no matching condition is required, it can be passed to 0
     */
